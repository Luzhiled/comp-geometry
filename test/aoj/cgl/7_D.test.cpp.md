...
data:
  ...
  bundledCode: "#line 1 \"test/aoj/cgl/7_D.test.cpp\"\n// verification-helper: PROBLEM\
    ...
    \   set(std::cerr, precision);\n    }\n  } iosetup;\n\n}\n#line 7 \"test/aoj/cgl/7_D.test.cpp\"\
    \n\n#line 9 \"test/aoj/cgl/7_D.test.cpp\"\n#include <algorithm>\n\nint main()\
    \ {\n  using R = long double;\n\n  geometry::circle<R> c;\n  std::cin >> c.o >>\
    \ c.r;\n\n  int q;\n  std::cin >> q;\n\n  while (q--) {\n    geometry::line<R>\
    \ l;\n    std::cin >> l.a >> l.b;\n\n    geometry::points<R> pts = geometry::cross_point_cl(c,\
  ...
  code: "// verification-helper: PROBLEM https://onlinejudge.u-aizu.ac.jp/problems/CGL_7_D\n\
    ...
    \    if (pts.size() == 1) {\n      pts.emplace_back(pts[0]);\n    }\n\n    if\
    \ (not compare_x(pts[0], pts[1])) std::swap(pts[0], pts[1]);\n\n    std::cout\
    \ << pts[0].x() << \" \" << pts[0].y() << \" \" << pts[1].x() << \" \" << pts[1].y()\
    \ << std::endl;\n  }\n}\n"
  dependsOn:
  ...
  requiredBy: []
  timestamp: '2023-06-01 02:01:33+09:00'
